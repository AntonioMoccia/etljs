...
/**
 * File-sonda: viene creato dentro un pacchetto, gli si fa importare qualcosa
 * di proibito e si verifica che dependency-cruiser se ne accorga. Cosi' I2 e I9
 * restano imposti dallo strumento anche se un giorno la configurazione cambia.
 */
const probes: string[] = [];

async function plantProbe(packageDir: string, source: string): Promise<void> {
  const path = join(repoRoot, "packages", packageDir, "src", "__boundary_probe__.ts");
  probes.push(path);
...
async function cruise(): Promise<{ code: number; output: string }> {
  try {
    const { stdout } = await exec("npx", ["depcruise", "packages", "--config", ".dependency-cruiser.cjs"], {
      cwd: repoRoot,
    });
...
describe("confini architetturali", () => {
  ...
  test("un plugin che importa il core fa fallire il controllo (I9)", async () => {
    await plantProbe("plugin-csv", 'import "@etl-js/core";\nexport const probe = 1;\n');
    const result = await cruise();
  ...
  test("un plugin che importa un altro plugin fa fallire il controllo (I9)", async () => {
    await plantProbe("plugin-csv", 'import "@etl-js/plugin-postgres";\nexport const probe = 1;\n');
    const result = await cruise();
    expect(result.code).not.toBe(0);
    expect(result.output).toContain("plugin-dipende-solo-da-contracts");
  }, 60_000);
  ...
  test("il core che importa un plugin fa fallire il controllo (I2)", async () => {
    await plantProbe("core", 'import "@etl-js/plugin-csv";\nexport const probe = 1;\n');
    const result = await cruise();
    expect(result.code).not.toBe(0);
  ...
  test("l'harness di test che importa il core fa fallire il controllo (I9)", async () => {
    await plantProbe("testing", 'import "@etl-js/core";\nexport const probe = 1;\n');
    const result = await cruise();
    expect(result.code).not.toBe(0);
  ...
  test("contracts che dipende da qualcosa fa fallire il controllo (I9)", async () => {
    await plantProbe("contracts", 'import "zod";\nexport const probe = 1;\n');
    const result = await cruise();
    expect(result.code).not.toBe(0);
